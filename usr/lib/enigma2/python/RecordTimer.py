...
#################################################################
	...
	def __init__(self, serviceref, begin, end, name, description, eit, disabled = False, justplay = False, afterEvent = AFTEREVENT.AUTO, checkOldTimers = False, dirname = None, tags = None, plugins = {}):
		...
		if serviceref is not None and serviceref.isRecordable():
	...
	def activate(self):
		...
		if next_state == self.StatePrepared:
			...
			if self.plugins:
				...
				for pname, (pval, pdata) in six.iteritems(self.plugins):
					if pval == 'True':
...
class RecordTimer(timer.Timer):
	...
	def saveTimer(self):
		...
		for timer in self.timer_list + self.processed_timers:
			...
			if timer.plugins:
				for key, (val, data) in six.iteritems(timer.plugins):
					if val is not 'False':
