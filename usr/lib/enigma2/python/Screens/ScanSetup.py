...
#				print "SR after round", sr_rounded
	...
	def cableTransponderSearchSessionClosed(self, *val):
		...
		# driver based scan...
		if self.updateStateTimer is not None:
			self.frontend.getStateChangeSignal().remove(self.frontendStateChangedCable)
...
#				parm.inversion = inv[data[5]]
	...
	def startCableTransponderSearch(self, nim_idx):
		...
		if tunername == "Si2169C":
			...
			if not self.frontend:
				...
				if not self.frontend:
					...
					if not self.frontend:
						...
						return
			self.__tlist = [ ]
			self.frontend.getStateChangeSignal().append(self.frontendStateChangedCable)
			...
			self.updateStateTimer.callback.append(self.updateStateCable)
			self.updateStateTimer.start(1000, True)
...
class SatelliteTransponderSearchSupport:
	def satelliteTransponderSearchSessionClosed(self, *val):
		if self.frontend:
			self.frontend.getStateChangeSignal().remove(self.frontendStateChangedSat)
			self.frontend = None
			self.channel = None
			self.updateStateTimer = None
...
#					print "SR after round", sr_rounded
	...
	def startSatelliteTransponderSearch(self, nim_idx, orb_pos):
			...
			if not self.frontend:
				...
				if not self.frontend:
					...
					if not self.frontend:
						print "couldn't allocate tuner %d for blindscan!!!" %nim_idx
						return
			self.frontend.getStateChangeSignal().append(self.frontendStateChangedSat)
		...
		if self.auto_scan:
			self.updateStateTimer = eTimer()
			self.updateStateTimer.callback.append(self.updateStateSat)
			self.updateStateTimer.start(1000, True)
...
class ScanSetup(ConfigListScreen, Screen, TransponderSearchSupport, CableTransponderSearchSupport, SatelliteTransponderSearchSupport, TerrestrialTransponderSearchSupport):
	...
	def createSetup(self):
		...
		if nim.isCompatible("DVB-S"):
			if self.scan_type.value == "single_transponder":
				...
				if nim.isCompatible("DVB-S2") and tunername != "Si2166B":
			...
			if self.scan_typeterrestrial.value == "single_transponder":
				...
				base_path = self.scan_ter if self.scan_ter.system.value == eDVBFrontendParametersTerrestrial.System_DVB_T else self.scan_ter2
	...
	def newConfig(self):
		...
		if cur == self.typeOfScanEntry or \
			...
			(self.plpidAutoEntry and self.systemEntry[1].value == eDVBFrontendParametersTerrestrial.System_DVB_T2 and cur == self.plpidAutoEntry):
			self.createSetup()
