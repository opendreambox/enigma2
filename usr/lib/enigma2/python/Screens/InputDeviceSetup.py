...
class InputDeviceSelection(Screen,HelpableScreen):
	...
	def buildInterfaceList(self,device,description,type ):
		...
		if type == 'remote':
			if enabled:
				devicepng = LoadPixmap(resolveFilename(SCOPE_CURRENT_SKIN, "skin_default/icons/input_rc%i-configured.png" % config.misc.rcused.value))
			else:
				devicepng = LoadPixmap(resolveFilename(SCOPE_CURRENT_SKIN, "skin_default/icons/input_rc%i.png" % config.misc.rcused.value))
		elif type == 'keyboard':
			if enabled:
